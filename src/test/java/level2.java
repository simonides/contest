...
public class level2 {
    ...
    public void setUp() throws Exception {
        input = new InputReader("level/level2_5.in");
        output = new OutputWriter("level/level2_5.out.txt");
    }
    ...
    public void execute() {
        ...
        for (int y = 0; y < rows; y++) {
            ...
            for (int x = 0; x < cols; x++) {
                ...
                final Integer country = integers1.get(x * 2 + 1);
                world.put(y, x, new LandCell(alt, country, x, y));
                if (alt < min) min = alt;
                if (alt > max) max = alt;
                total += alt;
            }
        ...
        for (long i = 0; i < Integer.MAX_VALUE; i++) {
            final Long count = borderCount.get(i);
            if (count == null) break;
            output.writeLine(String.format("%d", count));
        }
